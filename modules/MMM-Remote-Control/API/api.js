...
module.exports = {
	...
	getApiKey() {
		...
		if (typeof thisConfig !== "undefined" && "config" in thisConfig) {
			...
			if ("secureEndpoints" in thisConfig.config && !thisConfig.config.secureEndpoints) {
		...
		const getActions = function (content) {
			...
			if (re.test(content)) {
				content.match(re).forEach((match) => {
					...
					if (["ALL_MODULES_STARTED", "DOM_OBJECTS_CREATED", "KEYPRESS", "MODULE_DOM_CREATED", "KEYPRESS_MODE_CHANGED", "USER_PRESENCE"].indexOf(n) === -1) {
	...
	createApiRoutes() {
		...
		this.expressRouter.route(["/saves", "/classes", "/module/installed", "/module/available", "/brightness", "/translations", "/mmUpdateAvailable", "/config"]).get((req, res) => {
		...
		let route = expressVersionLessThan5
			? ["/refresh/:delayed?", "/shutdown/:delayed?", "/reboot/:delayed?", "/restart/:delayed?", "/save", "/minimize", "/togglefullscreen", "/devtools"]
			: ["/refresh{/:delayed}", "/shutdown{/:delayed}", "/reboot{/:delayed}", "/restart{/:delayed}", "/save", "/minimize", "/togglefullscreen", "/devtools"];

		this.expressRouter.route(route).get((req, res) => {
			if (!this.apiKey && this.secureEndpoints) {
				return res.status(403).json({ success: false, message: "Forbidden: API Key Not Provided in Config! Use secureEndpoints to bypass this message" });
			}
		...
		route = expressVersionLessThan5 ? "/userpresence/:value?" : "/userpresence{/:value}";
		this.expressRouter.route(route).get((req, res) => {
			if (req.params.value) {
		...
		route = expressVersionLessThan5 ? "/update/:moduleName?" : "/update{/:moduleName}";
		this.expressRouter.route(route).get((req, res) => {
			if (!this.apiKey && this.secureEndpoints) {
				return res.status(403).json({ success: false, message: "Forbidden: API Key Not Provided in Config! Use secureEndpoints to bypass this message" });
			}
		...
		route = expressVersionLessThan5 ? "/notification/:notification/:p?/:delayed?" : "/notification/:notification{/:p}{/:delayed}";
		this.expressRouter
			.route(route)
			.get((req, res) => {
				if (!this.apiKey && this.secureEndpoints) {
		...
		route = expressVersionLessThan5 ? "/module/:moduleName?/:action?/:delayed?" : "/module{/:moduleName}{/:action}{/:delayed}";
		this.expressRouter
			.route(route)
			.get((req, res) => {
		...
		route = expressVersionLessThan5 ? "/monitor/:action?/:delayed?" : "/monitor{/:action}{/:delayed}";
		this.expressRouter
			.route(route)
			.get((req, res) => {
		...
		this.expressRouter.route(["/brightness/:setting"]).get((req, res) => {
	...
	answerModuleApi(req, res) {
		...
		if (["SHOW", "HIDE", "FORCE", "TOGGLE", "DEFAULTS"].indexOf(action) !== -1) {
		...
		Object.values(this.externalApiRoutes).forEach((r) => {
			...
			Object.keys(r.actions).forEach((a) => {
				...
				if ("prettyName" in r.actions[a]) {
					...
					item.text = this.translate(r.actions[a].notification)
						.toLowerCase()
						.replace(/(^|_)(\w)/g, ($0, $1, $2) => ($1 && " ") + $2.toUpperCase());
				}
				sub.items.push(item);
			});
