...
const Remote = {
	...
	validPositions: ["", "top_bar", "top_left", "top_center", "top_right", "upper_third", "middle_center", "lower_third", "bottom_left", "bottom_center", "bottom_right", "bottom_bar", "fullscreen_above", "fullscreen_below"],
	savedData: {},
	translations: {},
	currentConfig: {},
	addModule: "",
	...
	socketNotificationReceived(notification, payload) {
		if (notification === "REMOTE_ACTION_RESULT") {
			...
			if ("success" in payload) {
				...
				const message = payload.status === "error" ? `${this.translate("RESPONSE_ERROR")}: <br><pre><code>${JSON.stringify(payload, undefined, 3)}</code></pre>` : payload.info;
	...
	showMenu(newMenu) {
		...
		if (newMenu === "main-menu") {
			this.loadList("config-modules", "config", (parent, configData) => {
				...
				if (!configData.modules.find((m) => m.module === "alert") && alertElem !== undefined) {
					alertElem.remove();
				}
				...
				if ((!modConfig || !modConfig.classes) && classesButton) {
	...
	addToggleElements(parent) {
		...
		const spanClasses = ["fa fa-fw fa-toggle-on outer-label fa-stack-1x", "fa fa-fw fa-toggle-off outer-label fa-stack-1x", "fa fa-fw fa-lock inner-small-label fa-stack-1x"];
		...
		self.loadToggleButton(moduleBox, (toggledOn, event) => {
			if (toggledOn) {
				if (self.hasClass(event.currentTarget, "external-locked")) {
					...
					const force = self.createSymbolText(
						"fa fa-warning",
	...
	appendModuleEditElements(wrapper, moduleData) {
		...
		for (let i = 0; i < moduleData.length; i++) {
			...
			const moduleBox = self.createSymbolText(
				"fa fa-fw fa-pencil",
	...
	createMenuElement(content, menu, insertAfter) {
		...
		if ((!window.location.hash && menu !== "main") || (window.location.hash && window.location.hash.substring(1) !== `${menu}-menu`)) {
