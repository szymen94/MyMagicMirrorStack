...
Module = {
	...
	register(name, moduleDefinition) {
		...
		Module.notificationHandler[name] = "notificationReceived" in moduleDefinition ? moduleDefinition.notificationReceived.toString() : "";
	}
};
...
module.exports = NodeHelper.create({
		...
		downloadModules({
			...
			callback: (result) => {
				if (result && result.startsWith("ERROR")) {
					Log.error("[MMM-Remote-Control]", result);
				}
				this.readModuleData();
			}
		});
	...
	answerGet(query, res) {
		...
		if (query.data === "classes") {
			...
			this.sendResponse(res, undefined, { query, data: thisConfig.classes ? thisConfig.classes : {} });
			return;
		}
		if (query.data === "saves") {
			const backupHistorySize = 5;
	...
	monitorControl(action, opts, res) {
		...
		const monitorOnCommand = this.initialized && "monitorOnCommand" in this.thisConfig.customCommand ? this.thisConfig.customCommand.monitorOnCommand : "vcgencmd display_power 1";
		const monitorOffCommand = this.initialized && "monitorOffCommand" in this.thisConfig.customCommand ? this.thisConfig.customCommand.monitorOffCommand : "vcgencmd display_power 0";
		const monitorStatusCommand = this.initialized && "monitorStatusCommand" in this.thisConfig.customCommand ? this.thisConfig.customCommand.monitorStatusCommand : "vcgencmd display_power -1";
		switch (action) {
			case "MONITORSTATUS":
				...
				break;
			case "MONITORTOGGLE":
				exec(monitorStatusCommand, opts, (error, stdout) => {
					status = offArr.indexOf(stdout.trim()) !== -1 ? "off" : "on";
					if (status === "on") {
	...
	shutdownControl(action, opts) {
		const shutdownCommand = this.initialized && "shutdownCommand" in this.thisConfig.customCommand ? this.thisConfig.customCommand.shutdownCommand : "sudo shutdown -h now";
		const rebootCommand = this.initialized && "rebootCommand" in this.thisConfig.customCommand ? this.thisConfig.customCommand.rebootCommand : "sudo shutdown -r now";
		if (action === "SHUTDOWN") {
		...
		if (query.action === "MANAGE_CLASSES") {
			...
			switch (typeof query.payload.classes) {
				case "string":
					...
					break;
				case "object":
					query.payload.classes.forEach((t) => classes.push(this.thisConfig.classes[t]));
			}
			classes.forEach((cl) => {
				for (const act in cl) {
	...
	checkForExecError(error, stdout, stderr, res, data) {
		if (error) {
			Log.error("[MMM-Remote-Control]", stderr);
		}
	...
	in(pattern, string) {
		return includes(pattern, string);
	},
